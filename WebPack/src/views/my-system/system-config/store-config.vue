    ...
    export default {
        ...
        methods: {
            ...
            delete(dict) {
                let vm = this;
                console.log(dict);
                sysCfgSrv.saveStoreDict(dict,"delete").then(function (result) {
                    if (result && result.data && result.data.status == '0') {
                        vm.storeDictList.splice(dict._index, 1);
            ...
            ok() {
                let vm = this;
                sysCfgSrv.saveStoreDict(vm.query).then(function (result) {
                    if (result && result.data && result.data.status == '0') {
                        vm.query = Object.assign({}, vm.query, result.data.data);
                        if (vm.isAdd) {
                            vm.storeDictList.push(vm.query);
                        } else {
                            vm.storeDictList.splice(vm.query._index, 1,vm.query);
                        }
                        vm.isShow = false;
