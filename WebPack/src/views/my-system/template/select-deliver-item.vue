    ...
    >
        <Form ref="formItem" :model="query" :label-width="80" :rules="rulesValidate">
            <FormItem label="选择仓库">
                ...
                >
                    <Option v-for="item in storeDictList" :key="item.serialNo" :value="item.serialNo"
                            :label="item.name">
                        <span>{{item.name}}</span>
                    </Option>
                </Select>
...
<script>
    ...
    export default {
        name:'select-deliver-item',
        watch: {
            item:function() {
                this.query = Object.assign({}, this.item);
            },
            "query.storeCode": function () {
                let query = {storeCode: this.query.storeCode}
