...
<template>
    <Card>
        ...
        <p slot="extra">
            <Button type="primary" @click="newDamage">新建</Button>
            <Button type="primary" @click="save">保存</Button>
            <Button type="primary" @click="ackDamage">确认报损</Button>
        </p>
        <Form ref="formMaster" :model="master" :rules="rulesValidate" :label-width="80">
            <Row>
                ...
                <Col span="12">
                <FormItem label="报损日期" prop="damageDateTime">
            ...
            <Row>
                <Col span="12">
                <FormItem label="报损仓库" prop="storeCode">
...
<script>
    ...
    import selectDefectiveDamageItem from "../template/select-defective-damage-item.vue";
    ...
    export default {
        ...
        data() {
            return {
                ...
                rulesValidate: {
                    damageDateTime: [
                        {required: true, type: 'date', message: '报损日期不能为空！', trigger: 'change'}
                    ],
                    storeCode: [
                        {required: true, type: 'string', message: '报损仓库不能为空！', trigger: 'change'}
                    ],
                },
            };
        },
        methods: {
            ...
            ackDamage() {
                let vm = this;
                storeMgrSrv.ackDefectiveDamage({documentNo: vm.master.documentNo}).then(function (result) {
            ...
            save() {
                ...
                vm.$refs.formMaster.validate((valid) => {
                    ...
                    console.log(vm.master);
                    if (valid) {
                        storeMgrSrv.saveDefectiveDamage(vm.master).then(function (result) {
                            if (result && result.data && result.data.id) {
                                vm.master = Object.assign({}, vm.master, result.data,{damageDateTime:new Date(result.data.deliverDateTime)});
            ...
            updateInfo() {
                ...
                if (query.documentNo != null && query.documentNo != undefined) {
                    ...
                    invokeSrv.getFnData(storeMgrSrv.getDefectiveDamageMasterList, query, function (data) {
                        console.log(data);
                        if (data != null && data.length == 1) {
                            vm.master = Object.assign(vm.master, data[0]);
                            invokeSrv.getFnData(storeMgrSrv.getDefectiveDamageDetailList, query, function (data) {
